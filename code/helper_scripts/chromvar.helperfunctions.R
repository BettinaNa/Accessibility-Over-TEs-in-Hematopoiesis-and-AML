...
dunn_test <- function(inputs, groups) {
    groups <- as.factor(groups)
    inputs <- na.omit(inputs)
    q12 <- apply(inputs, 1, function(e) dunn_test_helper(e, groups, 1))
    q13 <- apply(inputs, 1, function(e) dunn_test_helper(e, groups, 2))
    q23 <- apply(inputs, 1, function(e) dunn_test_helper(e, groups, 3))
    return(data.frame(repname=rownames(inputs) ,q_group12= q12, q_group13 = q13, q_group23 = q23))
}

dunn_test_helper <-  function(inputs, groups, row){
    templist <- dunnTest(inputs, groups, method="bh")
    q <- as.data.frame(templist[2])[row,4]
    return(q)
}


# ## Add medians to the differential repeats
differentialDeviations5=function(inputs,groups, alternative = c("two.sided", "less","greater"),  parametric = TRUE) {
  ...
  rownum=nrow(medians)
  mediandf <- as.data.frame(t(medians))
  colnames(mediandf) <-  levels(as.factor(groups))
  df <- data.frame(p_value = p_val, p_value_adjusted = p_adj, stringsAsFactors=F)
  df <- cbind(df,mediandf )
  return(df)
